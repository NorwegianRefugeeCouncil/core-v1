...
function createBaseFormValue(): FormValue {
  return { id: "", data: undefined };
}

export const FormValue = {
  encode(message: FormValue, writer: _m0.Writer = _m0.Writer.create()): _m0.Writer {
    if (message.id !== "") {
      writer.uint32(10).string(message.id);
    }
  ...
  decode(input: _m0.Reader | Uint8Array, length?: number): FormValue {
    const reader = input instanceof _m0.Reader ? input : new _m0.Reader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = createBaseFormValue() as any;
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
        case 1:
  ...
  fromJSON(object: any): FormValue {
    return { id: isSet(object.id) ? String(object.id) : "", data: isSet(object?.data) ? object.data : undefined };
  },

  toJSON(message: FormValue): unknown {
    const obj: any = {};
    message.id !== undefined && (obj.id = message.id);
  ...
  fromPartial<I extends Exact<DeepPartial<FormValue>, I>>(object: I): FormValue {
    const message = createBaseFormValue() as any;
    message.id = object.id ?? "";
