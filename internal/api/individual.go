...
type GetAllOptions struct {
	...
	CountryID                  string
	DisplacementStatuses       []DisplacementStatus
	Email                      string
	FullName                   string
	Genders                    []Gender
	IsMinor                    *bool
...
func (o GetAllOptions) QueryParams() template.HTML {
	...
	if len(o.DisplacementStatuses) != 0 {
		dsStrings := make([]string, len(o.DisplacementStatuses))
		for i, ds := range o.DisplacementStatuses {
			dsStrings[i] = string(ds)
		}
		params.Add("displacement_status", strings.Join(dsStrings, ","))
	}
	u := url.URL{
