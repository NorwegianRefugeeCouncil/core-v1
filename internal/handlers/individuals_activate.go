...
func HandleIndividualsActivate(repo db.IndividualRepo) http.Handler {
	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if len(invalidIndividualIds) > 0 {
			l.Warn("user trying to activate individuals that don't exist or are in the wrong country", zap.Strings("individual_ids", invalidIndividualIds))
			http.Error(w, fmt.Sprintf("individuals not found: %v", invalidIndividualIds), http.StatusNotFound)
			return
		}

		if err := repo.ActivateMany(ctx, individualIds); err != nil {
