...
func (i individualRepo) getAllInternal(ctx context.Context, tx *sqlx.Tx, options api.GetAllOptions) ([]*api.Individual, error) {
	...
	var allRet []*api.Individual

	var batchSize int
	if len(options.IDs) <= 0 {
		batchSize = maxParams
	} else {
		batchSize = maxParams / len(options.IDs)
	}

	if err := batch(batchSize, options.IDs, func(idsInBatch []string) error {
		l.Debug("getting batch of individuals", zap.Any("ids", idsInBatch))
