...
func Test_newGetAllIndividualsSQLQuery(t *testing.T) {
	...
	zeroTime := time.Time{}
	const defaultQuery = `SELECT * FROM individual_registrations WHERE deleted_at IS NULL AND inactive = false`
	tests := []struct {
		name     string
	...
	}{
		{
			...
			wantSql:  `SELECT * FROM individual_registrations WHERE deleted_at IS NULL AND inactive = false AND address ILIKE $1`,
			wantArgs: []interface{}{"%123 Main St%"},
		}, {
			name:     "ageFrom",
			args:     api.ListIndividualsOptions{AgeFrom: pointers.Int(18)},
			wantSql:  `SELECT * FROM individual_registrations WHERE deleted_at IS NULL AND inactive = false AND age >= $1`,
			...
			wantSql:  `SELECT * FROM individual_registrations WHERE deleted_at IS NULL AND inactive = false AND birth_date >= $1`,
			...
			wantSql:  `SELECT * FROM individual_registrations WHERE deleted_at IS NULL AND inactive = false AND cognitive_disability_level = $1`,
			...
			wantSql:  `SELECT * FROM individual_registrations WHERE deleted_at IS NULL AND inactive = false AND collection_administrative_area_1 = $1`,
			...
			wantSql:  `SELECT * FROM individual_registrations WHERE deleted_at IS NULL AND inactive = false AND collection_administrative_area_2 = $1`,
			...
			wantSql:  `SELECT * FROM individual_registrations WHERE deleted_at IS NULL AND inactive = false AND collection_administrative_area_3 = $1`,
			wantArgs: []interface{}{"area3"},
		}, {
			name:     "collectionOffice",
			args:     api.ListIndividualsOptions{CollectionOffice: "office"},
			wantSql:  `SELECT * FROM individual_registrations WHERE deleted_at IS NULL AND inactive = false AND collection_office = $1`,
			wantArgs: []interface{}{"office"},
		}, {
			name:     "collectionAgentName",
			args:     api.ListIndividualsOptions{CollectionAgentName: "agent"},
			wantSql:  `SELECT * FROM individual_registrations WHERE deleted_at IS NULL AND inactive = false AND collection_agent_name = $1`,
			wantArgs: []interface{}{"agent"},
		}, {
			name:     "collectionAgentTitle",
			args:     api.ListIndividualsOptions{CollectionAgentTitle: "title"},
			wantSql:  `SELECT * FROM individual_registrations WHERE deleted_at IS NULL AND inactive = false AND collection_agent_title = $1`,
			wantArgs: []interface{}{"title"},
		}, {
			name:     "collectionTimeFrom",
			args:     api.ListIndividualsOptions{CollectionTimeFrom: &someDate},
			wantSql:  `SELECT * FROM individual_registrations WHERE deleted_at IS NULL AND inactive = false AND collection_time >= $1`,
			wantArgs: []interface{}{&someDate},
		}, {
			...
			wantSql:  `SELECT * FROM individual_registrations WHERE deleted_at IS NULL AND inactive = false AND community_id = $1`,
			wantArgs: []interface{}{"communityId"},
		}, {
			name:     "countryId",
			args:     api.ListIndividualsOptions{CountryID: "countryId"},
			wantSql:  `SELECT * FROM individual_registrations WHERE deleted_at IS NULL AND inactive = false AND country_id = $1`,
			wantArgs: []interface{}{"countryId"},
		}, {
			name:     "createdAtFrom",
			args:     api.ListIndividualsOptions{CreatedAtFrom: &someDate},
			wantSql:  `SELECT * FROM individual_registrations WHERE deleted_at IS NULL AND inactive = false AND created_at >= $1`,
			wantArgs: []interface{}{&someDate},
		}, {
			name:     "createdAtTo",
			...
			wantSql:  `SELECT * FROM individual_registrations WHERE deleted_at IS NULL AND inactive = false AND displacement_status IN ($1,$2)`,
			...
			wantSql:  `SELECT * FROM individual_registrations WHERE deleted_at IS NULL AND inactive = false AND displacement_status IN ($1,$2,$3,$4,$5,$6,$7)`,
			...
			wantSql: `SELECT * FROM individual_registrations WHERE deleted_at IS NULL AND inactive = false`,
		}, {
			name:     "email",
			args:     api.ListIndividualsOptions{Email: "email"},
			wantSql:  `SELECT * FROM individual_registrations WHERE deleted_at IS NULL AND inactive = false AND (email_1 = $1 OR email_2 = $1 OR email_3 = $1)`,
			wantArgs: []interface{}{"email"},
		}, {
			name:     "freeField1",
			args:     api.ListIndividualsOptions{FreeField1: "freeField1"},
			wantSql:  `SELECT * FROM individual_registrations WHERE deleted_at IS NULL AND inactive = false AND free_field_1 = $1`,
			wantArgs: []interface{}{"freeField1"},
		}, {
			name:     "freeField2",
			args:     api.ListIndividualsOptions{FreeField2: "freeField2"},
			wantSql:  `SELECT * FROM individual_registrations WHERE deleted_at IS NULL AND inactive = false AND free_field_2 = $1`,
			wantArgs: []interface{}{"freeField2"},
		}, {
			name:     "freeField3",
			args:     api.ListIndividualsOptions{FreeField3: "freeField3"},
			wantSql:  `SELECT * FROM individual_registrations WHERE deleted_at IS NULL AND inactive = false AND free_field_3 = $1`,
			wantArgs: []interface{}{"freeField3"},
		}, {
			name:     "freeField4",
			args:     api.ListIndividualsOptions{FreeField4: "freeField4"},
			wantSql:  `SELECT * FROM individual_registrations WHERE deleted_at IS NULL AND inactive = false AND free_field_4 = $1`,
			wantArgs: []interface{}{"freeField4"},
		}, {
			name:     "freeField5",
			args:     api.ListIndividualsOptions{FreeField5: "freeField5"},
			wantSql:  `SELECT * FROM individual_registrations WHERE deleted_at IS NULL AND inactive = false AND free_field_5 = $1`,
			wantArgs: []interface{}{"freeField5"},
		}, {
			name:     "full name",
			args:     api.ListIndividualsOptions{FullName: "John"},
			wantSql:  `SELECT * FROM individual_registrations WHERE deleted_at IS NULL AND inactive = false AND (full_name ILIKE $1 OR preferred_name ILIKE $2 OR first_name ILIKE $3 OR middle_name ILIKE $4 OR last_name ILIKE $5 OR native_name ILIKE $6)`,
			...
			wantSql:  `SELECT * FROM individual_registrations WHERE deleted_at IS NULL AND inactive = false AND sex IN ($1,$2,$3,$4)`,
			...
			wantSql:  `SELECT * FROM individual_registrations WHERE deleted_at IS NULL AND inactive = false AND has_cognitive_disability = $1`,
			...
			wantSql:  `SELECT * FROM individual_registrations WHERE deleted_at IS NULL AND inactive = false AND has_communication_disability = $1`,
			wantArgs: []interface{}{true},
		}, {
			...
			wantSql:  `SELECT * FROM individual_registrations WHERE deleted_at IS NULL AND inactive = false AND has_consented_to_rgpd = $1`,
			wantArgs: []interface{}{true},
		}, {
			...
			wantSql:  `SELECT * FROM individual_registrations WHERE deleted_at IS NULL AND inactive = false AND has_mobility_disability = $1`,
			wantArgs: []interface{}{true},
		}, {
			...
			wantSql:  `SELECT * FROM individual_registrations WHERE deleted_at IS NULL AND inactive = false AND has_selfcare_disability = $1`,
			wantArgs: []interface{}{true},
		}, {
			...
			wantSql:  `SELECT * FROM individual_registrations WHERE deleted_at IS NULL AND inactive = false AND has_vision_disability = $1`,
			wantArgs: []interface{}{true},
		}, {
			...
			wantSql:  `SELECT * FROM individual_registrations WHERE deleted_at IS NULL AND inactive = false AND hearing_disability_level = $1`,
			wantArgs: []interface{}{"mild"},
		}, {
			name:     "householdId",
			args:     api.ListIndividualsOptions{HouseholdID: "household-id"},
			wantSql:  `SELECT * FROM individual_registrations WHERE deleted_at IS NULL AND inactive = false AND household_id = $1`,
			wantArgs: []interface{}{"household-id"},
		}, {
			name:     "id (single)",
			args:     api.ListIndividualsOptions{IDs: containers.NewStringSet("1")},
			wantSql:  `SELECT * FROM individual_registrations WHERE deleted_at IS NULL AND inactive = false AND id IN ($1)`,
			wantArgs: []interface{}{"1"},
		}, {
			name:     "id (multiple)",
			args:     api.ListIndividualsOptions{IDs: containers.NewStringSet("1", "2")},
			wantSql:  `SELECT * FROM individual_registrations WHERE deleted_at IS NULL AND inactive = false AND id IN ($1,$2)`,
			wantArgs: []interface{}{"1", "2"},
		}, {
			name:     "identificationNumber",
			args:     api.ListIndividualsOptions{IdentificationNumber: "123456789"},
			wantSql:  `SELECT * FROM individual_registrations WHERE deleted_at IS NULL AND inactive = false AND (identification_number_1 = $1 OR identification_number_2 = $1 OR identification_number_3 = $1)`,
			...
			wantSql:  `SELECT * FROM individual_registrations WHERE deleted_at IS NULL AND inactive = false AND engagement_context IN ($1)`,
			wantArgs: []interface{}{"inOffice"},
		}, {
			name:     "internalID",
			args:     api.ListIndividualsOptions{InternalID: "internal-id"},
			wantSql:  `SELECT * FROM individual_registrations WHERE deleted_at IS NULL AND inactive = false AND internal_id = $1`,
			wantArgs: []interface{}{"internal-id"},
		}, {
			name:     "isHeadOfCommunity",
			args:     api.ListIndividualsOptions{IsHeadOfCommunity: pointers.Bool(true)},
			wantSql:  `SELECT * FROM individual_registrations WHERE deleted_at IS NULL AND inactive = false AND is_head_of_community = $1`,
			wantArgs: []interface{}{true},
		}, {
			...
			wantSql:  `SELECT * FROM individual_registrations WHERE deleted_at IS NULL AND inactive = false AND is_head_of_household = $1`,
			wantArgs: []interface{}{true},
		}, {
			...
			wantSql:  `SELECT * FROM individual_registrations WHERE deleted_at IS NULL AND inactive = false AND is_female_headed_household = $1`,
			wantArgs: []interface{}{true},
		}, {
			...
			wantSql:  `SELECT * FROM individual_registrations WHERE deleted_at IS NULL AND inactive = false AND is_minor_headed_household = $1`,
			wantArgs: []interface{}{true},
		}, {
			...
			wantSql:  `SELECT * FROM individual_registrations WHERE deleted_at IS NULL AND inactive = false AND is_minor = $1`,
			wantArgs: []interface{}{true},
		}, {
			name:     "isMinor (false)",
			...
			wantSql:  `SELECT * FROM individual_registrations WHERE deleted_at IS NULL AND inactive = false AND mobility_disability_level = $1`,
			wantArgs: []interface{}{"mild"},
		}, {
			name:     "nationality",
			args:     api.ListIndividualsOptions{Nationality: "nationality"},
			wantSql:  `SELECT * FROM individual_registrations WHERE deleted_at IS NULL AND inactive = false AND (nationality_1 = $1 OR nationality_2 = $1)`,
			wantArgs: []interface{}{"nationality"},
		}, {
			name:     "phone number",
			args:     api.ListIndividualsOptions{PhoneNumber: "1234567890"},
			wantSql:  `SELECT * FROM individual_registrations WHERE deleted_at IS NULL AND inactive = false AND (normalized_phone_number_1 ILIKE $1 OR normalized_phone_number_2 ILIKE $1 OR normalized_phone_number_3 ILIKE $1)`,
			...
			wantSql:  `SELECT * FROM individual_registrations WHERE deleted_at IS NULL AND inactive = false AND preferred_contact_method = $1`,
			...
			wantSql:  `SELECT * FROM individual_registrations WHERE deleted_at IS NULL AND inactive = false AND preferred_communication_language = $1`,
			...
			wantSql:  `SELECT * FROM individual_registrations WHERE deleted_at IS NULL AND inactive = false AND prefers_to_remain_anonymous = $1`,
			wantArgs: []interface{}{true},
		}, {
			...
			wantSql:  `SELECT * FROM individual_registrations WHERE deleted_at IS NULL AND inactive = false AND presents_protection_concerns = $1`,
			wantArgs: []interface{}{true},
		}, {
			...
			wantSql:  `SELECT * FROM individual_registrations WHERE deleted_at IS NULL AND inactive = false AND selfcare_disability_level = $1`,
			wantArgs: []interface{}{"mild"},
		}, {
			name:     "spokenLanguage",
			args:     api.ListIndividualsOptions{SpokenLanguage: "language"},
			wantSql:  `SELECT * FROM individual_registrations WHERE deleted_at IS NULL AND inactive = false AND (spoken_language_1 = $1 OR spoken_language_2 = $1 OR spoken_language_3 = $1)`,
			wantArgs: []interface{}{"language"},
		}, {
			name:     "updatedAtFrom",
			args:     api.ListIndividualsOptions{UpdatedAtFrom: pointers.Time(time.Date(2020, 1, 1, 0, 0, 0, 0, time.UTC))},
			wantSql:  `SELECT * FROM individual_registrations WHERE deleted_at IS NULL AND inactive = false AND updated_at >= $1`,
			...
			wantSql:  `SELECT * FROM individual_registrations WHERE deleted_at IS NULL AND inactive = false AND vision_disability_level = $1`,
			wantArgs: []interface{}{"mild"},
		}, {
			name:    "skip",
			args:    api.ListIndividualsOptions{Skip: 10},
			wantSql: `SELECT * FROM individual_registrations WHERE deleted_at IS NULL AND inactive = false OFFSET 10`,
		}, {
			name:    "take",
			args:    api.ListIndividualsOptions{Take: 10},
			wantSql: `SELECT * FROM individual_registrations WHERE deleted_at IS NULL AND inactive = false LIMIT 10`,
		}, {
			name:    "skip and take",
			args:    api.ListIndividualsOptions{Skip: 10, Take: 10},
			wantSql: `SELECT * FROM individual_registrations WHERE deleted_at IS NULL AND inactive = false OFFSET 10 LIMIT 10`,
		}, {
			name:    "sort (asc)",
			args:    api.ListIndividualsOptions{Sort: api.SortTerms{{Field: "id", Direction: api.SortDirectionAscending}}},
			wantSql: `SELECT * FROM individual_registrations WHERE deleted_at IS NULL AND inactive = false ORDER BY id ASC`,
		}, {
			name:    "sort (desc)",
			args:    api.ListIndividualsOptions{Sort: api.SortTerms{{Field: "id", Direction: api.SortDirectionDescending}}},
			wantSql: `SELECT * FROM individual_registrations WHERE deleted_at IS NULL AND inactive = false ORDER BY id DESC`,
		}, {
			name: "sort (multiple)",
			...
			wantSql: `SELECT * FROM individual_registrations WHERE deleted_at IS NULL AND inactive = false ORDER BY id ASC, full_name DESC`,
		},
	}
